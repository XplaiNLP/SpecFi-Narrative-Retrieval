...
client_ollama = openai.OpenAI(api_key="sk-proj-this-is-not-a-key", 
...
from taxonomy.co import taxonomy
...
text_embeddings_local = model_local.encode(texts, batch_size=batch_size, convert_to_tensor=True, show_progress_bar=True)
...
if RUN_OPTION == 'NodeRAG':
    ...
    config = NodeConfig.from_main_folder(r"./noderag_co_training")
...
def noderag_hyde(query, n_hyde=1):
    ...
    text = units_raw[1]
    parsed_items = [re.sub(r'^\d{1,2}\.\s*', '', line) for line in text.split('\n') if re.match(r'^\d{1,2}\.', line)]
...
def get_hyde_emb(query):
    query_embedding = get_openai_embeddings(query)[0]
    sims = torch.nn.functional.cosine_similarity(query_embedding.unsqueeze(0), text_embeddings_local, dim=1)
    topk_scores, topk_indices = torch.topk(sims, k=1)
    topk_indices = topk_indices.tolist()
    retrieved_claims = [texts[i] for i in topk_indices]
...
if RUN_OPTION == 'DR':
    ...
    data_path_training = "pn_training.csv"
...
def create_examples(hypos):
    examples = "\n\n"
    for el in hypos:
        ex_text = hypos[el].replace("\n", "")
        ex_text = ex_text.strip()
        new_ex = "Narrative: " + el + "\n" + "Text: " + ex_text + "\n\n"
        examples += new_ex
    return examples

if RUN_OPTION == 'NodeRAG' or RUN_OPTION == 'DR':
    examples = create_examples(all_hypos)
    logger.debug(examples)

if RUN_OPTION == 'STATIC':
    """static few shot test"""
    data_path_training = "data/training/test.csv"
    training_data_training = pd.read_csv(data_path_training)
    claims_training = training_data['claim'].tolist()
    texts_training = training_data['text'].tolist()
    claims_done = []
    all_examples_dyn_few_shot = {}
    for i, el in enumerate(claims_training):
        if el not in claims_done and el != "0_0":
            narrative_id = el
            narrative = taxonomy[narrative_id][0]
            all_examples_dyn_few_shot[narrative] = ""
            claims_done.append(el)
            all_examples_dyn_few_shot[narrative] = texts_training[i]
    print(all_examples_dyn_few_shot)
    examples = create_examples(all_examples_dyn_few_shot)
    print(examples)


def get_hyde_scores(query, n_hyde=3):
    ...
    for emb in hyde_embeddings:
        sims = torch.nn.functional.cosine_similarity(emb.unsqueeze(0), text_embeddings_local, dim=1)
...
for i in range(n):
    ...
    for narrative_id, subnarrative in tqdm(taxonomy.items(), desc=f"Processing {run_name}"):
        ...
        narr_count = 0
        logger.info(f"Narratives processed: {narratives_processed}, Narrative ID: {narrative_id}")

        sims, hyde_docs = get_hyde_scores(subnarrative, n_hyde=10)
        ...
        ap = average_precision_score(y_true_for_ap, y_scores_for_ap.tolist()) if total_relevant > 0 else 0.0
        ap_scores.append(ap)
        ...
        if r_precision_weighted_pairs:
            ...
            run_row[f"{narrative_id}_Weighted_R-Precision"] = total_tp / total_rel if total_rel > 0 else 0.0
    ...
    run_row['Overall_Weighted_R-Precision'] = total_tp_weighted / total_rel_weighted if total_rel_weighted > 0 else 0.0
    logger.debug(run_row['Overall_Weighted_R-Precision'])


    for k in Ks:
        run_row[f"Overall_P@{k}"] = np.mean(overall_metrics['precision'][k]) if overall_metrics['precision'][k] else 0.0
        run_row[f"Overall_R@{k}"] = np.mean(overall_metrics['recall_at_k'][k]) if overall_metrics['recall_at_k'][k] else 0.0
        run_row[f"Overall_NDCG@{k}"] = np.mean(overall_metrics['ndcg'][k]) if overall_metrics['ndcg'][k] else 0.0
